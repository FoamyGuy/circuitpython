...
//|
//|         The pin signature mirrors `I2SOut` so users can swap classes; recording
//|         parameters mirror `PDMIn`.
//|
...
//|           with audiobusio.I2SIn(board.D9, board.D10, board.D11,
...
static mp_obj_t audiobusio_i2sin_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    ...
    audiobusio_i2sin_obj_t *self = mp_obj_malloc_with_finaliser(audiobusio_i2sin_obj_t, &audiobusio_i2sin_type);
    common_hal_audiobusio_i2sin_construct(self, bit_clock, word_select, data, main_clock,
...
//|         ...
//|
static mp_obj_t audiobusio_i2sin_obj_record(mp_obj_t self_obj, mp_obj_t destination, mp_obj_t destination_length) {
    audiobusio_i2sin_obj_t *self = MP_OBJ_TO_PTR(self_obj);
    ...
    uint8_t bit_depth = common_hal_audiobusio_i2sin_get_bit_depth(self);
    ...
    uint32_t length_written =
        common_hal_audiobusio_i2sin_record_to_buffer(self, bufinfo.buf, length);
    return MP_OBJ_NEW_SMALL_INT(length_written);
}
MP_DEFINE_CONST_FUN_OBJ_3(audiobusio_i2sin_record_obj, audiobusio_i2sin_obj_record);

//|     sample_rate: int
//|     """The actual sample rate of the recording. This may not match the constructed
//|     sample rate due to internal clock limitations."""
//|
static mp_obj_t audiobusio_i2sin_obj_get_sample_rate(mp_obj_t self_in) {
    audiobusio_i2sin_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);
    return MP_OBJ_NEW_SMALL_INT(common_hal_audiobusio_i2sin_get_sample_rate(self));
}
MP_DEFINE_CONST_FUN_OBJ_1(audiobusio_i2sin_get_sample_rate_obj, audiobusio_i2sin_obj_get_sample_rate);

MP_PROPERTY_GETTER(audiobusio_i2sin_sample_rate_obj,
    (mp_obj_t)&audiobusio_i2sin_get_sample_rate_obj);

//|     bit_depth: int
//|     """The actual bit depth of the recording. (read-only)"""
//|
//|
static mp_obj_t audiobusio_i2sin_obj_get_bit_depth(mp_obj_t self_in) {
    audiobusio_i2sin_obj_t *self = MP_OBJ_TO_PTR(self_in);
    check_for_deinit(self);
    return MP_OBJ_NEW_SMALL_INT(common_hal_audiobusio_i2sin_get_bit_depth(self));
}
MP_DEFINE_CONST_FUN_OBJ_1(audiobusio_i2sin_get_bit_depth_obj, audiobusio_i2sin_obj_get_bit_depth);

MP_PROPERTY_GETTER(audiobusio_i2sin_bit_depth_obj,
    (mp_obj_t)&audiobusio_i2sin_get_bit_depth_obj);

static const mp_rom_map_elem_t audiobusio_i2sin_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR___del__), MP_ROM_PTR(&audiobusio_i2sin_deinit_obj) },
    ...
    { MP_ROM_QSTR(MP_QSTR_sample_rate), MP_ROM_PTR(&audiobusio_i2sin_sample_rate_obj) },
    { MP_ROM_QSTR(MP_QSTR_bit_depth), MP_ROM_PTR(&audiobusio_i2sin_bit_depth_obj) },
};
